...
from hfdb.queries import \
  get_regular_season_leaders, \
  get_player_data, \
  get_regular_season_stat_by_player_key_and_category, \
  get_playoff_leaders, \
  get_playoff_stat_by_player_key_and_category
...
class LeagueLeadersPlayoff(Resource):
  def get(self, date=None):
    leader_data = get_playoff_leaders(date)
    player_keys = []
    ...
    return jsonify({
      'leaderboard_type': 'playoff',
      'leaders': serialized
    })


class LeagueLeaderComparisonPlayoff(Resource):
  def get(self, player_key):
    season = request.args.get('season')
    category = request.args.get('category')

    comparison_stat_models = get_playoff_stat_by_player_key_and_category(player_key, category, season)
    if len(comparison_stat_models) == 0:
      comparison_stat_value = 0
    else:
      comparison_stat_model = comparison_stat_models[0]
